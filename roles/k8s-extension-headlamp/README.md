...
Uninstalls the `trivy-operator` release, then the `headlamp` release, via the shared `tasks/helm.yaml`. Note the monitoring play (`playbooks/homelab.yaml`) runs regardless of `uninstall`, letting the shared Helm task decide what to remove.