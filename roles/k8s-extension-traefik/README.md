...
- **PROXY protocol** — `web`/`websecure` listen on NodePorts `30080`/`30443` with proxy protocol enabled, so the HAProxy reverse proxy can forward client IPs (see [reverse-proxy](../reverse-proxy/README.md)).
...
- A running cluster (`k8s-core`) and the `kubernetes` tooling.
